...
if [ -d "$LOG_CLIENT_SRC" ]; then
  ...
  run_step "logging-client-maven" docker run --rm \
...
if [ "$HPDS_NEED_BUILD" = "true" ]; then
  ...
  run_step "pic-sure-hpds-maven" docker run --rm \
    -v "$HPDS_SRC:/src:ro" \
    -v "$MAVEN_CACHE:/root/.m2" \
  ...
  run_step "pic-sure-hpds-docker" docker build -f "$HPDS_SRC/docker/pic-sure-hpds/Dockerfile" \
    -t "hms-dbmi/pic-sure-hpds:$IMAGE_TAG" "$HPDS_BUILD_DIR"
  info "Built hms-dbmi/pic-sure-hpds:$IMAGE_TAG"

  run_step "pic-sure-hpds-etl-docker" docker build -f "$HPDS_BUILD_DIR/docker/pic-sure-hpds-etl/Dockerfile" \
