...
load_rdbms() {
  ...
  while [ "$#" -gt 0 ]; do
    case "$1" in
      --sql-properties) sql_properties="$2"; shift 2 ;;
...
hydrate_dictionary() {
  ...
  docker run --rm \
    ...
    "hms-dbmi/pic-sure-hpds-etl:${PICSURE_IMAGE_TAG:-LATEST}"
  curl_data -s -X POST -H "Content-Type: application/json" \
...
load_dictionary_csv() {
  ...
  if [ "$clear" = "true" ]; then
    curl_data -s -X DELETE http://dictionaryetl:8086/clear/all
  fi
  docker run --rm --network "$(network_name data)" \
    -v "$workdir/datasets.csv:/datasets.csv:ro" \
    curlimages/curl:latest \
    -s -X PUT -T /datasets.csv http://dictionaryetl:8086/api/dataset/csv
  ...
  while IFS=, read -r ref _; do
    ...
    docker run --rm --network "$(network_name data)" \
      ...
      curlimages/curl:latest \
      -s --request PUT --header "Content-Type: text/plain" \
...
load_facets() {
  ...
  docker run --rm --network "$(network_name data)" --entrypoint sh \
    ...
    curlimages/curl:latest -c '
      curl -s --request PUT --header "Content-Type: text/plain" --data-binary @/facet_categories.csv http://dictionaryetl:8086/api/facet/category/csv &&
      curl -s --request PUT --header "Content-Type: text/plain" --data-binary @/facets.csv http://dictionaryetl:8086/api/facet/csv &&
      curl -s --request PUT --header "Content-Type: text/plain" --data-binary @/facet_concepts.csv http://dictionaryetl:8086/api/facet/concept/csv
    '
  picsure_compose exec -T dictionary-db psql dictionary picsure -c 'UPDATE dict.update_info SET last_updated = NOW();' || true
  stop_dictionary_etl
  trap - EXIT
